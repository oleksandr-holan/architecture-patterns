...
internal abstract class Command(Input input, IValidate validator)
{
    protected Input Input { get; } = input;
    protected IValidate Validator { get; } = validator;

    public abstract bool Execute();
}

internal class Validate(Input input, IValidate validator) : Command(input, validator)
{
    public override bool Execute()
    {
        try
        {
            Validator.Handle(Input.Text);
            return true;
        }
        catch (Exception e)
        {
...
internal static class Program
{
    ...
    private static void Main()
    {
        ...
        var validateCommands = inputs.Select(inp => new Validate(inp, validator)).ToList();

        for (var i = 0; i < inputs.Count; i++)
        {
            var inp = inputs[i];
            var validateCommand = validateCommands[i];
